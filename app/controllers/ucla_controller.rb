class UclaController < ApplicationController
  def home
    ...
    calendar_feed_addr = 'http://www.google.com/calendar/feeds/icabruins@gmail.com/public/full?orderby=starttime&sortorder=ascending&futureevents=true&singleevents=true'

    # Check if GData client successfully gets data from the calendar feed
    begin
      feed = client.get(calendar_feed_addr).to_xml
    rescue
      puts 'UclaController#home: error getting gdata client calendar feed, address=' + calendar_feed_addr
    end
